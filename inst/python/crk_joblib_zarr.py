# -*- coding: utf-8 -*-
"""
Created on Fri Oct 10 05:42:29 2025
...
the zarr file and calculates kernels in parallel
using joblib. Best performance will be with a solid state drive.
Updated from original joblib script to use unordered 
generator for lower memory use and faster processing
@author: pj276
"""
...
def main() -> None:
    ...
    # Output distance array zarr store name
    # This is a temporary directory and gets deleted at the end of the script,
    ...
    if memReq > gbThreshold:
        ...
        res = Parallel(n_jobs=nThreads, return_as="generator_unordered", prefer="threads") (
            delayed(czf.calcKernelsMod)(i, dazarr, dThreshold, tForm, tkv, kvol) for i in range(0, len(sources))
            )
        ...
        for result in res:
            ccArr += result
            if kernelCount % 1000 == 0:
                toc1 = time.perf_counter()
                print(f'Finished corridor {kernelCount}', flush=True)
