# -*- coding: utf-8 -*-
"""
Created on Thu Apr 10 12:44:20 2025
...
#    ocs[(hvP==1) & (cpecells == 0)] = -9999
    ...
    cf.arrayToGeoTiff(cpecells2, 'C:/Users/pj276/Downloads/prioritization5/edge_cell_groups6.tif', profilecs)
    markers2 = np.expand_dims(markers, axis=0)
    cf.arrayToGeoTiff(markers2, 'C:/Users/pj276/Downloads/prioritization5/markers_groups6.tif', profilecs)
    labels2 = np.expand_dims(labels, axis=0)
    cf.arrayToGeoTiff(labels2, 'C:/Users/pj276/Downloads/prioritization5/labels_groups6.tif', profilecs)
    ...
    polyList = []
    for pid, pp in enumerate(ecc): # ecc is patch pairs
        ...
        dsp = pdrs[0] + pdrs[1]
        # Threshold      
        dsp = np.where(dsp < np.min(dsp[dsp>0]) + corrTol*2, dsp, 0)
        # Set nan to 0
        dsp[np.isnan(dsp)] = 0
        
        # Use patches to erase edges
        dsp = np.where(hvP==1, 0, dsp)
        
        # Segment corridors between edges
        csegs, nc1 = ndi.label(dsp, structure=fprint)
        
        # Loop through corridors
        if nc1 > 0:
            for n in range(nc1):
                # Get target corridor
                dsp2 = np.where(csegs==n+1, dsp, 0)
                
                # Minimum cost distance value between sets of points
                mcd = np.min(dsp2[dsp2>0])
                meancd = np.mean(dsp2[dsp2>0])
                
                # Mask out corridor where it overlaps with patches
                #dsp = np.where(np.logical_and(hvP > 1, dsp > 0), 0, dsp)
                dsp2 = np.where(hvP == 1, 0, dsp2)
                
                # Get corridor strength values overlapping with corridor
                tCorr = np.where(dsp2 > 0, lcc, 0)
                ...
                pl1 = pLabels.index(pp[0])
                pl2 = pLabels.index(pp[1])
                ...
                # Add to list
                attList.append(np.array([centx,centy,pp[0],pp[1],n+1,pAreas[pl1],pAreas[pl2],pMax[pl1],pMax[pl2],pMean[pl1],pMean[pl2],ppSum[pl1],ppSum[pl2],mcd,meancd,tCorrMax,tCorrMean,tCorrSum]))
            
    # Combine attributes into single array
    attArray = np.vstack(attList)
    # Shapefile column names
    column_names = ['xco','yco','pid1','pid2','cnum','parea1','parea2','pmax1','pmax2','pmean1','pmean2','psum1','psum2','mincost','meancost','maxstrn','meanstrn','sumstrn']
