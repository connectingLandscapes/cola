...
def main() -> None:
    ...
    def batchesCalcNew(reO, maxL):
        # Indicate whether row has been matched or not
        comp = np.zeros(len(reO))
        # Id array
        batchid = np.full(len(reO), -1, dtype='int')
        #batchid = np.zeros(len(reO))
        # Get match format for pairs
        nrows, ncols = reO.shape
        ddtype={'names':['f{}'.format(i) for i in range(ncols)],
               'formats':ncols * [reO.dtype]}
        # Empty arrays to hold pairs and unique nodes
        cc = np.array([]).astype('int')
        dd = np.empty((0,2), dtype='int')
        for i in range(0, len(reO)):
            # If there's no batch id associated with the 
            # row, add it to the current batch
            # and give it an ID
            if comp[i] == 0:      
                # Stack rows
                dd = np.vstack((dd, reO[i,:]))
                # Get unique nodes across rows and columns
                # and return as 1d array
                cc = np.unique(np.hstack((cc, np.unique(dd))))
                #comp[i] = 1 # mark as completed
                # Once we reach the max number of nodes allowed,
                # append the current batch to a list
                if len(cc) >= maxL:
                    tix = np.zeros(len(reO))
                    # Search the list for rows with
                    # these node pairs
                    # Get indices of completed pairs
                    pncx = list(combinations(cc, 2))
                    pncx = [np.array(i) for i in pncx]
                    pncx = np.vstack(pncx)
                    nmx = np.intersect1d(reO.view(ddtype), pncx.view(ddtype), return_indices=True)
                    # nmx1 are indices of reO where nodebatch pairs == reO
                    # we need the subset of these where comp is 0
                    tix[nmx[1]] = 1
                    batchid[np.logical_and(tix==1,comp==0)] = i
                    comp[tix==1] = 1
                    cc = np.array([]).astype('int')
                    dd = np.empty((0,2), dtype='int')
                elif (i == len(reO)-1) and (len(cc) < maxL):
                    tix = np.zeros(len(reO))
                    # Search the list for rows with
                    # these node pairs
                    # Get indices of completed pairs
                ...
                # we need the subset of these where comp is 0
                tix[nmx[1]] = 1
                batchid[np.logical_and(tix==1,comp==0)] = np.max(np.unique(batchid))+1
                comp[tix==1] = 1
                cc = np.array([]).astype('int')
                dd = np.empty((0,2), dtype='int')
        return comp, batchid
    ...
    if memReq > gbCLimit2:
        ...
        #%%
        # Calculate new groups of node pairs for processing
        comp, grp = batchesCalcNew(reOremain, nodeLimit)
        ...
        # and process
        for g, tg in enumerate(np.unique(grp)):
            print('Working on group ' + str(g+1) + ' of ' + str(len(np.unique(grp))))
            tic1 = time.perf_counter()
            # Pairs of corridors
            batch = reOremain[grp==tg]
            # Get unique sources from aggregated node list
            sourceBatch = np.unique(batch)
