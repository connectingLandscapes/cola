# -*- coding: utf-8 -*-
"""
Created on Thu Jun 12 14:47:51 2025
...
def main() -> None:
    ...
    # Get raster bounds and crs for fishnet
    ...
    if (nvc > 20e6) and (nck/nvc*100 < 50):
        ...
        # Polygon Size (5000 cells x cell dimension)
        ...
        # Make the "fishnet" one big cell covering the raster
        x, y = (minX, minY)
        geom_array = []
        geom = shapely.geometry.Polygon([(x,y), (x, maxY), (maxX, maxY), (maxX, y), (x, y)])
        geom_array.append(geom)
        fishnet = gpd.GeoDataFrame(geom_array, columns=['geometry']).set_crs(rasterCRS)
    ...
    # Loop through fishnet tiles and process if there's data
    for wi in range(len(fishnet)):
        # Get tile
        geom = []
        coord = shapely.geometry.mapping(fishnet)["features"][wi]["geometry"]
        geom.append(coord)
        
        # Get buffered tile
        geomb = []
        coord = shapely.geometry.mapping(fishnet.iloc[wi].geometry.buffer(dThreshold, cap_style="square"))
        geomb.append(coord)
        ...
        if np.sum(out_image > 0):
            ...
            print("working on tile " + str(wi))
            # Write unbuffered tile to temp file
            wtemp_ub = str(Path(ofile).parent / Path(tempfile.mktemp()).name) + "_UB_" + str(wi) + '.tif'
            with rio.open(wtemp_ub, 'w',**out_meta) as dest:
               dest.write(out_image, 1)
               
            # Buffered tile
            with rio.open(rg)as src:
                out_image2, out_transform2 = mask(src,geomb,crop=True)
                out_image2 = out_image2.squeeze()
                out_meta2 = src.meta
            ...
            # Write buffered tile to temp file
            wtemp_b = str(Path(ofile).parent / Path(tempfile.mktemp()).name) + "_B_" + str(wi) + '.tif'
            with rio.open(wtemp_b, 'w', **out_meta2) as dest:
               dest.write(out_image2, 1)
       
            # Create edge list from buffered window
            edges, nodeids, idmap = cf.generate_edges(out_image2, cSize)
            ...
            with rio.open(wtemp_b) as src:
                ...
                cinds = cf.cell_indices_from_coords(src, out_image2, np.array(xy))
                ...
                if np.sum(np.isnan(cinds)) >= 1:
                    ...
                    # Outside outer dims
                    if any(cinds[:,0] >= out_image2.shape[0]):
                        cinds = cinds[cinds[:,0] < out_image2.shape[0]]
                    if any(cinds[:,1] >= out_image2.shape[1]):
                        cinds = cinds[cinds[:,1] < out_image2.shape[1]]
                    # Outside inner dims
                    if any(cinds[:,0] < 0):
                    ...
                    checkND = out_image2[np.array(cinds[:,0], dtype=int),np.array(cinds[:,1], dtype=int)]
                    if -9999 in checkND:
                    ...
                    # Convert to list
                    sources = [(i[0]*out_image2.shape[1])+i[1] for i in cinds]
            ...
            dArr = np.zeros([out_image2.shape[0]*out_image2.shape[1]], 'float32')
            # Update with kernel values
            dArr[nodeids] = ccArr
            del ccArr
            # Reshape into rectangular array
            dArr = dArr.reshape(out_image2.shape[0],out_image2.shape[1])
            ...
            cf.arrayToGeoTiff(dArr, ofileTile, out_meta2)    
