...
setup_cola <- function( envName = 'cola', nSteps = 5, force = FALSE,
  ...
  #
  # envName = 'cola'; nSteps = 5; force = FALSE; yml = FALSE; onlyIndividual = F; ask = FALSE; dss = TRUE; zarr = T; spyder = TRUE; cola2 = FALSE; oldestPyVer = '3.11'; pyVer = "3.12.11"; updateCola = FALSE
  ...
  if ( dir.exists(miniPath) & is.null(miniBase) ){
    ...
    if( is.null(miniBase) ){
      ...
      if(ask){
        user_permission <- utils::askYesNo("Reinstall miniconda? downloads 80MB and takes some minutes")
      }

      if(user_permission){
        cat ('  We are forcing miniconda reinstall ... ')
        reticulate::install_miniconda(force = TRUE)
      } else {
        cat ('  Try forcing miniconda reinstalling: \n\t\t reticulate::install_miniconda(force = TRUE) \n' )
      }
      stop()
    }
  }
  ...
  if (is.null(condaLists)){
    ...
    if( class(condaLists) == 'data.frame'){
      if(any(grep('Anaconda|anaconda', condaLists$python))){
        warning(' STOP: Conda manager is Anaconda, not miniconda.',
                ' Install miniconda by "reticulate::install_miniconda( force = TRUE)"')
      }
    }
  }
  ...
  if( is.null(pyBaseVersion) ){
    message('We can´t check Python version for base conda environment.')
    ## List conda after instaling
    diagnose_cola()
    stop()
  } else {

    ## Check base conda base version, update if older than 3.11
    ...
    if ( zarr & numPyVers <= as.numeric(oldestPyVer) ){
    ...
    if( !envName %in% condaLists$name ){
      ...
      if ( isTRUE( user_permission ) ) {
        ...
        message(paste0('You need to run ´conda_create("', envName,'", python_version = "',
                       pyVer,'")´ before using this package'))
        stop()
      }
      ...
      cat (sep = '', '    ´', envName, '´ conda environment installed in ', cola::adaptFilePath(colaexe), '\n')
      (pyCola <- tryCatch( subset(condaLists, name == envName)$python, error = function (e) NULL) )
      (pyColaVersion <- tryCatch(system( paste0( pyCola, ' -V') , intern = TRUE), error = function(e) NULL))
      (pyVer2 <- gsub('Python|python| ', '', pyColaVersion))

      ## Change to specifc cpla version 3.12.11
      if ( pyVer2 != pyVer ){
      #(pyVer2 <- as.numeric( gsub('Python|python|\\.', '', pyColaVersion)) )
      #if ( pyVer2 != as.numeric( gsub('\\.', '', pyVer) ) ){

        (colaDir <- dirname(subset(condaLists, name == envName)$python))
        warning(paste0('   We suggest you to a use the ', pyVer ,' Python version. \n',
                       '   To update your CoLa`s Python version, please run the following commands and try `setup_cola()` again:\n',
                       '\n\treticulate::conda_remove(envname = "cola") \n',
                       '\tfile.remove("', colaDir, '")\n',
                       '\treticulate::conda_update()\n',
                       '\tsetup_cola(pyVer = "', pyVer , '")\n'
        ))

        ## Update and change to "3.12.11"
        if( updateCola) {
          if (ask){
            user_permission <- utils::askYesNo(
              paste0("Uninstall and update '", envName, "' conda environment? We prefer to have the ", pyVer, " version",
                     " but you have ", pyColaVersion, ". It migth take some minutes. 'Yes' for proceed if your version is older, ",
                     "'No' to keep the the existing one, and 'Cancel' to quit the installation"))
          }

          if ( isTRUE(user_permission) ) {
            cat (sep = '', '    Updating your Cola version to ', pyVer , ' ... please wait\n')
  ...
  if ( !onlyIndividual ){
    ...
    for(i in 1:3){
      ...
      (lib2inst <-as.character( na.omit( sapply(lib2inst, function(x){
        # x = lib2inst[1]

        (lib2 <- gsub('==.+', '', x))
        (av2 <- grep(lib2, avLibs$package, value = FALSE))
        if (length(av2) != 0){ # found but
          (isinstalled <- ifelse( substr(avLibs$requirement[av2], 0,
                                         nchar(gsub('==', '=', x))) == gsub('==', '=', x),
                                 yes = NA, # installed
                                 no  = x)) # no installed
        } else {
  ...
  (libs2inst <-as.character( na.omit( sapply(libs2inst, function(x){
    # x = lib2inst[1]
    (lib2 <- gsub('==.+', '', x))
    (av2 <- grep(lib2, avLibs$package, value = FALSE))
    if (length(av2) != 0){ # found but
  ...
  if( length(libs2inst) != 0){
    ...
    cat (
      paste0('\n From the ',length(libs2Install), ' libraries, ', length(libs2inst),
             ' are not installed yet: ', paste0(libs2inst, collapse = ', ') )
    )

    for( l in 1:length(libs2inst)){ # l = 10
      (lib2inst <- libs2inst[l]) # lib2inst = 'networkit==11.0'
      (lib2 <- gsub('==.+', '', lib2inst))
      (lib3 <- sub('=', '', lib2inst))
      # Check if specific
  ...
  # noInsLibs <- c('a', 'f')
  (noInsLibs <- na.omit(unlist(sapply(noInsLibs, function(x){
    # x = lib2inst[1]
    (lib2 <- gsub('==.+', '', x))
    (av2 <- grep(lib2, avLibs$package, value = FALSE))
    if (length(av2) != 0){ # found but
  ...
  (cmd2test <- paste0( #'cd ', cola_scripts_path, '; ',
    cola::quotepath(pyCola), ' ', cola::quotepath(welcomepy))); #cat(tryBcmd)
  (cmdans <- tryCatch( system( cmd2test , intern = TRUE ), error = function (e) e$message)) ## error is character
  # (system2(command =  cmd2test, receive.console.signals = TRUE ,  ))
  ...
  if ( any(grep('WELCOME ', cmdans)) ){
    ...
    (test_suit2res <- paste0(
      cola::quotepath(pyCola), ' ', # Python
      cola::quotepath(pyScript), ' ', # script
      cola::quotepath(system.file("sampledata/sampleTif.tif", package = "cola")), ' ', #in [1]
      cola::quotepath(outTest), #out
    ...
    if ( dir.exists(cola_scripts_path) & file.exists(outTest) ){
      ...
      # (pos <- ifelse(length(pos) == 0, length(Renviron) + 1, pos))
      if (length(pos) == 0){Renviron[length(Renviron) + 1] <- paste0('COLA_SERVER=', 0)}
      ...
      if (Sys.getenv("RSTUDIO") == "1"){
        cat (sep = '', '\n',
             '    You can run .rs.restartR() for this\n')
        .rs.restartR()
      }

      if (Sys.getenv("RSTUDIO") == "1" & !ask){
        cat (sep = '', '\n',
             '    Restarting ... .rs.restartR() \n')
        .rs.restartR()
      }
...
install_conda_env <- function(envName, useYML = FALSE, ymlFile = NULL,
  ...
  if( useYML ){
    ...
    if( any(grep('Could not solve for environment specs', insCondLog)) ){
      ...
      insCondLog <- tryCatch(
        conda_create(envname = envName, packages = packagess, python_version = pv),
        error = function(e) e$message)
    }
