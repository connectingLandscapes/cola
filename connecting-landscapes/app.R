...
## Init A
{
  ...
  # mkdir /data; sudo chown -R shiny:shiny /data; sudo chown -R shiny:shiny /home/shiny//home/shiny/connecting-landscapes
  logFilePath <<- base::paste0(rootPath, '/cola_logFolders.txt')
  
  base::source(base::paste0(gitPath, '/R/cola_tools.R'))
  hs2rs_file <- base::paste0(gitPath, '/docs/HS_size5_nd_squared.tif')
  ...
  showcasePath <<- base::paste0(gitPath, '/showcase/')
  ...
  base::load(base::paste0(gitPath, '/docs/per_uper_crs_df.RData'))  
  ...
  allLogs <- base::list.files(path = path_error,
                              pattern = 'cola|connec')
  validLogs <- unname(
    na.omit(
  ...
  sh_object <- base::paste0(gitPath, '/R/showcase.RData')
...
server <- function(input, output, session) {
  ...
  replaceRastShp <- function(polDraw, burnval, rastPath, rastCRS = NA, lineBuffW = 2){
  ...
  updateLL <- function(ll){
    output$ll_map_cprA <- output$ll_map_cprB <- output$ll_map_cpr1 <- output$ll_map_cpr2 <- 
      output$ll_map_pri <- output$ll_map_lcc <- output$ll_map_crk <- output$ll_map_map <- 
      output$ll_map_plot <- output$ll_map_edi <- output$ll_map_dist <-
  ...
  updateParamsTEXT <- function(params_txt = params_txt, 
                               start = FALSE,
                               hs = FALSE, sr = FALSE, pts = FALSE,
                               lcc = FALSE, crk = FALSE, pprint = FALSE){
    ...
    pts <<- params_txt_loc$pts <- (pts | params_txt_loc$pts)
    lcc <<- params_txt_loc$lcc <- (lcc | params_txt_loc$lcc)
    crk <<- params_txt_loc$crk <- (crk | params_txt_loc$crk)
    
    params_txt <<- params_txt_loc
    
    # if( pprint){
    #   print('params_txt during')
    #   print(rbind(do.call(cbind, params_txt),do.call(cbind, params_txt_loc)))
    # }
    ...
    if( pts ){
      output$out_par_lccA <- output$out_par_crkA <- renderText({ paste0("Input: Points", readyMsg)})  
    }
    ...
    # output$out_par_surA <- renderText({ 
    #   paste0("Input: Habitat suitability", 
    #          ifelse(hs, readyMsg, notReadyMsg))
    # })
    # 
    # output$out_par_ptsA <- renderText({paste0("Input: Raster ", 
    #                                           ifelse(hs|sr, readyMsg, notReadyMsg))})
    # 
    # output$out_par_prioA <<- output$out_par_ediA <<- output$out_par_lccB <<- output$out_par_crkB <<- 
    #   renderText({paste0("Input: Surface resistance", 
    #                                              ifelse(sr, readyMsg, notReadyMsg))})
    # 
    # output$out_par_lccA <- output$out_par_crkA <- renderText({ 
    #   paste0("Input: Points", ifelse(pts, readyMsg, notReadyMsg))
    #   })
    # 
    # output$out_par_prioB <- renderText({ paste0("Input: Corridors", 
    #                                             ifelse(lcc, readyMsg, notReadyMsg))})
    # 
    # output$out_par_prioC <- renderText({ paste0("Input: Kernels", 
    #                                             ifelse(crk, readyMsg, notReadyMsg))})
    # if( pprint){
    #   print('params_txt after ')
    #   print(rbind(do.call(cbind, params_txt),do.call(cbind, params_txt_loc)))
    # }
    
    return(params_txt_loc)
    # paste0("Input: Habitat suitability", Msg)
    # paste0("Input: Surface resistance", Msg)
    # paste0("Input: Raster", Msg)
    # paste0("Input: Corridors", Msg)
    # paste0("Input: Kernels", Msg)
    # output$out_par_surA <- renderText({ paste("Input: Habitat suitability<font color=\"",color_darkred_hex,"\"><b> Not ready</b></font>") })
    # output$out_par_ediA <- renderText({ paste("Input: Surface resistance<font color=\"",color_darkred_hex,"\"><b> Not ready</b></font>") })
    # output$out_par_ptsA <- renderText({ paste("Input: Raster<font color=\"",color_darkred_hex,"\"><b> Not ready</b></font>") })
    # output$out_par_lccA <- renderText({ paste("Input: Points<font color=\"",color_darkred_hex,"\"><b> Not ready</b></font>") })
    # output$out_par_lccB <- renderText({ paste("Input: Surface resistance<font color=\"",color_darkred_hex,"\"><b> Not ready</b></font>") })
    # output$out_par_crkA <- renderText({ paste("Input: Points<font color=\"",color_darkred_hex,"\"><b> Not ready</b></font>") })
    # output$out_par_crkB <- renderText({ paste("Input: Surface resistance<font color=\"",color_darkred_hex,"\"><b> Not ready</b></font>") })
    # output$out_par_prioA <- renderText({ paste("Input: Surface resistance<font color=\"",color_darkred_hex,"\"><b> Not ready</b></font>") })
    # output$out_par_prioB <- renderText({ paste("Input: Least cost paths<font color=\"",color_darkred_hex,"\"><b> Not ready</b></font>") })
    # output$out_par_prioC <- renderText({ paste("Input: Kernels<font color=\"",color_darkred_hex,"\"><b> Not ready</b></font>") })
  }
  ...
  params_txt <- updateParamsTEXT(params_txt = params_txt, pprint = FALSE, start = TRUE)
  #print(params_txt)
  ...
  observeEvent(input$in_sur_tif, {
    ...
    output$ll_map_h2r <- leaflet::renderLeaflet({
      ...
      if(is.na(newtifPath)){
        ...
        params_txt <- updateParamsTEXT(params_txt = params_txt, hs = TRUE, sr = TRUE)
        rv$newtifPath <- newtifPath
        rv$hs <- newtifPath
        rv$hsready <- TRUE
    ...
    if(rv$hsready){
      ...
      output$ll_map_h2r <- leaflet::renderLeaflet({
        ...
        if(!is.na(hs2rs_file)){
          ...
          params_txt <- updateParamsTEXT(params_txt = params_txt, sr = TRUE, hs = TRUE)
  ...
  ## Edit scenarios replacing values
  ...
  observeEvent(input$points_py, {
    if(! (rv$tifready | rv$hsready)){
      ...
      if (!file.exists(points_file)){
        ...
        params_txt <- updateParamsTEXT(params_txt = params_txt, 
                                       pts = TRUE, 
                                       hs = in_points_ly == 'HabitatSuitability',
                                       sr = in_points_ly == 'SurfaceResistance'
        )
        rv$pts <- points_file
        rv$ptsready <- TRUE
...

{
  ...
  ")),
        shinydashboard::tabItems(
          ...
          # tab_corridors tab_kernels tab_plotting tab_Mapping
          # tab_priori tab_genetics tablocal           
            ...
            fluidPage(
              ...
              tabsetPanel(
                ...
                tabPanel(
                  "Home", 
                  includeMarkdown(
                    file.path(gitPath, '/docs/md_intro.md')
                  )),
                tabPanel(
                  "How it works", 
                  includeMarkdown(
                    file.path(gitPath, '/docs/md_use.md')
                  )),
                ...
                tabPanel("Showcase",
                         includeMarkdown(
                           file.path(gitPath, '/docs/md_showcase.md')),
                         fluidRow(
                           # img(src=file.path(gitPath, 'showcase.gif')
